...
Система обеспечивает полный цикл обработки — от загрузки изображений до анализа результатов. В изолированной СУБД сохраняются данные сотрудников: ФИО, табельный номер, отдел и другие данные пользователей, загружающих фотографии.

Проект полностью построен на открытом программном обеспечении — все Python-библиотеки загружены из официального репозитория PyPI.
...
2. **Авторизация** — перейдите на сайт Photo Server (`http://localhost:8001`) и введите полученный логин/пароль, указанные при регистрации в AeroToolKit, в форму.
...
4. **Загрузка изображений** — на странице Photo Server загрузите и отправьте несколько фотографий инструментов. В локальных контейнерах сервис работает очень быстро, но на виртуальной машине Yandex Cloud наблюдается снижение скорости обработки HTTP-запросов. На обоих серверах ("AeroToolKit Backend" и "Photo Server") реализованы Celery — распределенные системы для выполнения фоновых задач в Python-приложениях, которые обеспечивают асинхронную обработку данных, фоновое выполнение операций и масштабирование вычислений.
...
6. **Техническое примечание** — в текущей версии сервисы развернуты на Gunicorn с Whitenoise для обслуживания статических файлов. Конфигурация оптимизирована для разработки и работает в отладочном режиме (DEBUG=True). Помимо этого, как указанно выше - серверы яндекса вносят свою задержку при обработке HTTP-запросов.
...
- **Scalable Background Jobs** - параллельная обработка задач с поддержкой повторных попыток и отказоустойчивостью
