...
1. Зарегистрироваться на [AeroToolKit](http://aerotoolkit.sytes.net:8000/). Локальный адрес AeroToolKit http://127.0.0.1:8000/ если сайт уже не работает на Yandex Cloud
2. Ввести логин/пароль от [AeroToolKit](http://aerotoolkit.sytes.net:8000/) на [Photo Server](http://aerotoolkit.sytes.net:8001/), НАСТРОИТЬ ПАРАМЕТРЫ РАСПОЗНАВАНИЯ (!), загрузить и отправить фотографии из папки photo_for_demonstration. Локальный адрес Photo Server http://127.0.0.1:8001/ если сайт уже не работает на Yandex Cloud
3. Вернуться обратно на [AeroToolKit](http://aerotoolkit.sytes.net:8000/) и посмотреть результат работы модели компьютерного зрения. Все ссылки на сайте кликабельны, т.о. можно изучить весь список обращений к сайту, либо посмотреть отдельную запись каждого сотрудника.
